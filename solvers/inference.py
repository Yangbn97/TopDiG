...
def run_inference(args):
    ...
    for filename in tqdm(img_list):
        ...
        if args.configs['Experiment']['object_type'] == 'line':
            FullDiGraph, subgraphs, model_time_tmp = predict_line_graph_WithOverlap_simple(args, img, patch_size=300, intersect_area=0)
            ...
            patch_size = 512 if args.configs['Experiment']['dataset_name'] == 'GID' else 300

            shapely_polygons, model_time_tmp = predict_poly_graph_WithOverlap_simple(args, img, patch_size=patch_size, intersect_area=100)
